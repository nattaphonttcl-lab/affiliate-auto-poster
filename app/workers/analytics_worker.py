...
from app.services.publishing_workers import AnalyticsSyncWorker
...
def _run_once() -> None:
    ...
    with SessionLocal() as db:
        ...
        worker = AnalyticsSyncWorker(service)
        worker.run_once(owner_user_id=settings.workers_default_owner_user_id)


def main() -> None:
    settings = get_runtime_settings()
    configure_logging(settings.log_level, json_logs=settings.log_json)
    run_forever("analytics-worker", _run_once, settings)
