...
from app.services.publishing_workers import RetryWorker
...
def _run_once() -> None:
    ...
    with SessionLocal() as db:
        ...
        worker = RetryWorker(service)
        worker.run_once(worker_id="retry-worker")


def main() -> None:
    settings = get_runtime_settings()
    configure_logging(settings.log_level, json_logs=settings.log_json)
    run_forever("retry-worker", _run_once, settings)
