...
from app.schemas.caption import CaptionGenerateRequest, CaptionGenerateResponse, CaptionItemRead, CaptionStyle
from app.services.caption_service import CaptionService

router = APIRouter(prefix="/captions", tags=["captions"])


@router.post("/generate", response_model=CaptionGenerateResponse, status_code=status.HTTP_200_OK)
def generate_captions(
    payload: CaptionGenerateRequest,
    _: int = Depends(get_current_user_id),
    service: CaptionService = Depends(get_caption_service),
) -> CaptionGenerateResponse:
    batch, captions = service.generate_for_product(product_id=payload.product_id, style=payload.style)
