...
class DashboardRepository:
    ...
    def recent_caption_batches(self, limit: int = 10) -> list[CaptionBatch]:
        stmt = select(CaptionBatch).order_by(CaptionBatch.created_at.desc()).limit(limit)
        return list(self._db.scalars(stmt))

    def recent_promotional_images(self, limit: int = 10) -> list[PromotionalImage]:
        stmt = select(PromotionalImage).order_by(PromotionalImage.created_at.desc()).limit(limit)
        return list(self._db.scalars(stmt))
